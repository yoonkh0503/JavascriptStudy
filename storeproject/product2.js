class Book {
    constructor(name, price, writer, pageNum, publisher, stockNum) {
        this.name = name;
        this.price = price;
        this.writer = writer;
    ...
    selling() {
        console.log(this.price + "만큼 가격이 차감되었습니다.");
        console.log("수량이 차감되었습니다.");
    }
};

class ForeignBook extends Book {
    constructor(name, price , writer , pageNum , translater, publisher,stockNum) {
        super(name, price, writer, pageNum, publisher,stockNum);
...
class Ebook extends Book {
    constructor(name, price, writer, pageNum, publisher, devices, stockNum) {
        super(name, price, writer, pageNum, publisher, stockNum);
...
const book1 = new Book("자바의정석", 30000, "남궁성", 1300, "도우출판",1);
// const book2 = new Book("Head First Python", 20000, "paul bedder", 700); // 해외 책이라서, 번역자도 필요함.
const book2 = new ForeignBook("Head First Python", 20000, "paul bedder", 700, "우정은", "한빛미디어",2);
const book3 = new Ebook("니체는 나체다.", 8900, "유영만", 400 ,"생각속의 집", ["아이폰","아이패드","안드로이드","PC"],3);
...
const paintProduct = function() {
    ...
    for(let i = 0; i < bookList.length; i++) {
        ...
        option.value = bookList[i].stockNum;
...
/*
Summary : 2021-09-14 BookStore Project
----------------------------------------------------------------
type : Feat
Description :
----------------------------------------------------------------
