...
const makeRankingObj = function(val) {
    localStorage.setItem(`match${playCnt}`,val);
};
...
const rankingCalculate = function() {
   ...
   makeRankingObj(carList) // value 값은 String으로 저장된다.
   return carList;
};
...
- radio 버튼을 미리 만들어두지 말고, 전적검색 페이지로 이동하고 로드가 됐을 때 각 게임 회차별로 radio 버튼이 생성되도록.
- radio 값을 받아서 localStorage에 저장되어 있는 각 회차 별 랭킹을 보여주고 싶음.
*/
...
- 전적검색 페이지는 불완전한 게 많음. 계속해서 보완할 것.
- radio 버튼을 미리 만들어두지 말고, 전적검색 페이지로 이동하고 로드가 됐을 때 각 게임 회차별로 radio 버튼이 생성되도록.
- radio 값을 받아서 localStorage에 저장되어 있는 각 회차 별 랭킹을 보여주고 싶음.
*/